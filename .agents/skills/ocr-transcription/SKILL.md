...
- **الاستطلاع المسبق عبر `pdf-inspector` (Pre-flight Inspection)**: يُشغَّل الوكيل الرئيسي سكربت [`pdf_inspect.py`](scripts/pdf_inspect.py) قبل تخيير المستخدم لاستخلاص ملف التصنيف (`text-based` / `scanned` / `image-based` / `mixed`) وحقول `pages_needing_ocr` و`is_rtl` و`has_tables` في `output_parts/inspection.json`، ثم يعرض التوصية وفق مصفوفة القرار الموثَّقة في **[references/pdf_inspection_rules.md](references/pdf_inspection_rules.md)**. يظل استخلاص النصوص حصرًا عبر أداة `view_file` للنموذج؛ فالأداة تكشف بنية الـ PDF ولا تستخرج نصه.
- **تثبيت دليل العمل**: لا يُبدَّل دليل العمل (`Cwd`) أثناء دورة التقسيم والتحقق والدمج والتحويل، ويُحدَّد وفقًا للآتي:
  - يُفحص جذر مساحة العمل أولًا؛ فإن وُجد مجلد مشروع قائم يحتوي على البنية الأساسية للمهارة (مثل مجلد `ocr workflow` الذي يضم مجلد `pdf/` أو `scripts/`)، يُعتمد هذا المجلد مباشرة كدليل للعمل (`Cwd`).
  - في حال عدم وجود مجلد مشروع قائم وكون مساحة العمل غير فارغة، يُنشأ مجلد فرعي باسم `cwd\ocr workflow` ويُضبط كدليل للعمل (`Cwd`) تفاديًا لتلوث جذر مساحة العمل.
  - في حال خلو مساحة العمل تمامًا من أي ملفات أو مجلدات، يُعتمد جذر مساحة العمل مباشرة كدليل للعمل (`Cwd`) دون إنشاء مجلدات فرعية.
